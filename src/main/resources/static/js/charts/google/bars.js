...
function drawBar() {
	...
	var riskada = [];
	$.ajax({
		url : 'http://localhost:9000/admin/allQuestions/test',
		type : 'POST',
		data : {},
		async : false,
		success : function(data) {
			if (!data.includes("UNKNOWN REQUEST")) {

				riskada = $.parseJSON(data);
	...
	/*var data = google.visualization
			.arrayToDataTable([ [ 'Year', 'Sales', 'Expenses' ],
					[ '2004', 1000, 400 ], [ '2005', 1270, 460 ],
					[ '2006', 660, 1220 ], [ '2007', 1030, 540 ] ]);*/
	...
	// Draw chart
	var bar = new google.visualization.BarChart($('#none')[0]);
...
function drawBarStacked() {
	...
	// Draw chart
	var bar_stacked = new google.visualization.BarChart(
			$('#google-bar-stacked')[0]);
...
function drawColumnStacked() {
	...
	// Draw chart
	var column_stacked = new google.visualization.ColumnChart(
			$('#google-column-stacked')[0]);
